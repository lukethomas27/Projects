...
void led_state(uint8_t LED, uint8_t state) {
	DDRL = 0xFF;
	if(state!=0){//if the state is not 0 then set the LED given to on
		if(LED==0){
			PORTL |= 0b10000000;
		}else if(LED==1){
			PORTL |= 0b00100000;
		...
		if(LED==0){ //otherwise turn off the LED given
			PORTL &= 0b01111111;
		}else if(LED==1){
			PORTL &= 0b11011111;
		}else if(LED==2){
...
void SOS() {
	...
	for(int i=0; i<length; i++){
			if(light[i]==1){ //for only one led turning on for given duration. So this is the "dot"
				led_state(0,1);
				_delay_ms(duration[i]);
				led_state(0,0);
			}else if(light[i]==0){ //delays led for given duration
				_delay_ms(duration[i]);
			}else{ //sets all 4 leds for the given duration so this is the "dash"
				led_state(0,1);
				led_state(1,1);
				led_state(2,1);
				led_state(3,1);
...
void glow(uint8_t LED, float brightness) {
	...
	while(1){ //inf loop 
		if(count<threshold){ //if below threshold set LED
			led_state(LED, 1);
		}else if(count<PWM_PERIOD){ //if below PWM_PERIOD reset LED
			led_state(LED, 0);
		}else{ //else set LED
			count=0;
			led_state(LED, 1);
...
void pulse_glow(uint8_t LED) {
	while(1){	//infinite loop
		count = 0;
		slow_count = 0;
		for (int threshold = 0; threshold <= PWM_PERIOD; threshold = slow_count*0.1 ){	//this loop increases brightness 
			if (count < threshold){
				led_state(LED,1);
		...
		count = 0;
		slow_count = 0;
		for (int threshold = 0; threshold <= PWM_PERIOD; threshold = slow_count*0.1 ){ //this loop decreases brightness
			if (count < threshold){
				led_state(LED,0);
			}else if (count < PWM_PERIOD){
				led_state(LED,1);
...
void light_show() {
	uint8_t light[] = {
		0b00001111, 0b00000000, 0b00001111, 0b00000000, 0b00001111, 0b00000000, 0b00000110, 0b00000000, 
		0b00001001, 0b00000000, 0b00001111, 0b00000000, 0b00001111, 0b00000000, 0b00001111, 0b00000000, 
		0b00001001, 0b00000000, 0b00000110, 0b00000000,0b00001000, 0b00001100, 0b00000110, 0b00000011,
		0b00000001, 0b00000011, 0b00000110, 0b00001100, 0b00001000, 0b00001100, 0b00000110, 0b00000011, 
		0b00000001, 0b00000011, 0b00000110, 0b00001111, 0b00000000, 0b00001111, 0b00000000, 0b00000110, 
		0b00000000, 0b00000110, 0b00000000
	};

	int duration[] = {
		250, 250, 250, 250, 250, 250, 100, 100, 100, 100, 250, 250,
		250, 250, 250, 250, 100, 100, 100, 100, 100, 100, 100, 100,
		100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 250,
