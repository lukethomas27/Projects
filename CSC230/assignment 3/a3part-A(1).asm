...
.def DH=r25  ;DATAH:DATAL  store 10 bits data from ADC
.def BH = r1
.def DL=r24
.def BL = r0

ldi r16, low(RAMEND)
mov BL, r21 ;low boundary
ldi r16, high(RAMEND)
mov BH, r21 ;high boundary
...
start: ;polling function to start timers
	in r16, TIFR3
	sbrs r16, OCF3A
	rjmp start
...
timer1: ;self explanatory timer1 
	push r16
	lds r16, SREG
	push r16
	push DL
...
timer3: ;timer3 calls star if button is pressed otherwise maintains dash
	push r16
	push r17
	push r18
	ldi r16, 1
...
dash:	;dash occurs when button is pressed is 0
	ldi r18, '-'
	push r18
	rcall lcd_putchar
	pop r18
...
star:	;star occurs when button is pressed is 1
	ldi r18, '*'
	push r18
	rcall lcd_putchar
	pop r18
